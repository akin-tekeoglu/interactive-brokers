...
class LevelTwoHeatmap:
    ...
    def __init__(self,length) -> None:
        ...
        self.im = self.ax.imshow(self.order_history,aspect='auto',extent=[0,self.length,0,20],vmin=0,vmax=100)
        labels = ["bid10", "bid9", "bid8", "bid7", "bid6", "bid5", "bid4","bid3","bid2","bid1",
              "ask1", "ask2", "ask3", "ask4", "ask5", "ask6","ask7","ask8","ask9","ask10"]
        self.ax.set_yticks(np.arange(len(labels)), labels=labels)
        self.ax.set_title("Limit orders")
        self.fig.tight_layout()
        self.colorbar=self.fig.colorbar(self.im, ax=self.ax, label='Interactive colorbar')
